...
(function ($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function getTextCallback(doneChangingTextCallback) {
        debugger;
        doneChangingTextCallback(prompt('Enter Lesion Name:'));
    }

    function changeTextCallback(data, doneChangingTextCallback) {
        //doneChangingTextCallback(prompt('Change Lesion Name:'));
    }
    ...
    function addNewMeasurement(mouseEventData) {
        ...
        // since we are dragging to another place to drop the end point, we can just activate
        // the end point and let the moveHandle move it for us.
        $(mouseEventData.element).off('CornerstoneToolsMouseMove', cornerstoneTools.ellipticalRoi.mouseMoveCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDown', cornerstoneTools.ellipticalRoi.mouseDownCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDownActivate', cornerstoneTools.ellipticalRoi.mouseDownActivateCallback);

        cornerstone.updateImage(mouseEventData.element);
        cornerstoneTools.moveNewHandle(mouseEventData, toolType, measurementData, measurementData.handles.end, function () {
            //cornerstoneTools.moveNewHandle(mouseEventData, toolType, measurementData, measurementData.handles.orthoEnd, function () {
            if (cornerstoneTools.anyHandlesOutsideImage(mouseEventData, measurementData.handles)) {
                // delete the measurement
                cornerstoneTools.removeToolState(mouseEventData.element, toolType, measurementData);
            }
            ...
            if (measurementData.text === undefined || measurementData.text == "") {
                config.getTextCallback(doneChangingTextCallback);
            }

            $(mouseEventData.element).on('CornerstoneToolsMouseMove', eventData, cornerstoneTools.ellipticalRoi.mouseMoveCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDown', eventData, cornerstoneTools.ellipticalRoi.mouseDownCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDownActivate', eventData, cornerstoneTools.ellipticalRoi.mouseDownActivateCallback);
            cornerstone.updateImage(mouseEventData.element);
        });
    }
    ...
    function addNewMeasurementTouch(touchEventData) {
        ...
        function doneChangingTextCallback(text) {
            ...
            measurementData.active = false;
            measurementData.imageno = (stack.currentImageIdIndex + 1);
            cornerstone.updateImage(element);
        }

        cornerstoneTools.addToolState(element, toolType, measurementData);
        $(element).off('CornerstoneToolsTouchStartActive', cornerstoneTools.ellipticalRoiTouch.touchDownActivateCallback);
        $(element).off('CornerstoneToolsTap', cornerstoneTools.ellipticalRoiTouch.tapCallback);
        cornerstone.updateImage(element);
        ...
        cornerstoneTools.moveNewHandleTouch(touchEventData, toolType, measurementData, measurementData.handles.end, function () {
            ...
            $(element).on('CornerstoneToolsTouchStartActive', cornerstoneTools.ellipticalRoiTouch.touchDownActivateCallback);
            $(element).on('CornerstoneToolsTap', cornerstoneTools.ellipticalRoiTouch.tapCallback);
        });
    }
    ...
    function createNewMeasurement(mouseEventData) {
        ...
        measurementData.text = NTLMark;
        measurementData.imageno = (stack.currentImageIdIndex + 1); //Added by Bhargav Thaker //13Mar2023
        return measurementData;
    }
    ///////// END ACTIVE TOOL ///////
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            var textX, textY, area, meanStdDev, SUV; //SUV Added by Bhargav Thaker
            ...
            if (!data.invalidated) {
                ...
                area = data.area;
                SUV = data.SUV; //SUV Added by Bhargav Thaker //23Feb2023
            } else {
            ...
            if (data.text != "" && data.text != undefined) {
                //cornerstoneTools.drawTextBox(context, 'NL: ' + data.text, textX, textY + fontHeight + 25, color); //Commented by Bhargav Thaker

                // Added By Bhargav Thaker Started
                if (eventData.image.data.string('x00080060') === 'PT') {
                    cornerstoneTools.drawTextBox(context, 'NL: ' + data.text, textX, textY + fontHeight + 45, color);
                    ...
                    var storedPixels;
                    if (eventData.image.color) {
                        storedPixels = cornerstoneTools.getRGBPixels(eventData.element, x, y, width, height);
                    }
                    else {
                        storedPixels = cornerstone.getStoredPixels(eventData.element, x, y, width, height);
                    }
                    var sp = storedPixels[0];
                    //SUV = cornerstoneTools.calculateSUV(eventData.image, eventData.image.maxPixelValue - eventData.image.minPixelValue);
                    SUV = cornerstoneTools.calculateSUV(eventData.image, sp);

                    if (SUV !== undefined && !isNaN(SUV)) {
                        var SUVText = 'SUV: ' + SUV.toFixed(3);
                        cornerstoneTools.drawTextBox(context, SUVText, textX, textY + fontHeight + 25, color);
                        toolData = cornerstoneTools.getToolState(e.currentTarget, toolType);
                    }
                }
                else {
                    cornerstoneTools.drawTextBox(context, 'NL: ' + data.text, textX, textY + fontHeight + 25, color);
                }
                // Added By Bhargav Thaker Ended
            }
            context.restore();
        }
    }
    ...
    function pressCallback(e, eventData) {
        ...
        function doneChangingTextCallback(data, updatedText, deleteTool) {
            ...
            data.active = false;
            cornerstone.updateImage(element);
            $(element).on('CornerstoneToolsTouchStart', cornerstoneTools.ellipticalRoiTouch.touchStartCallback);
            $(element).on('CornerstoneToolsTouchStartActive', cornerstoneTools.ellipticalRoiTouch.touchDownActivateCallback);
        }
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            if (pointNearTool(element, data, coords)) {
                ...
                cornerstone.updateImage(element);
                // Allow relabelling via a callback
                $(element).off('CornerstoneToolsTouchStart', cornerstoneTools.ellipticalRoiTouch.touchStartCallback);
                $(element).off('CornerstoneToolsTouchStartActive', cornerstoneTools.ellipticalRoiTouch.touchDownActivateCallback);
                config.changeTextCallback(data, doneChangingTextCallback);
    ...
    cornerstoneTools.ellipticalRoi.setConfiguration(configuration);

    cornerstoneTools.ellipticalRoiTouch = cornerstoneTools.touchTool({
        addNewMeasurementTouch: addNewMeasurementTouch,
        createNewMeasurement: createNewMeasurement,
        onImageRendered: onImageRendered,
...
(function ($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function addNewMeasurement(mouseEventData) {
        ...
        // since we are dragging to another place to drop the end point, we can just activate
        // the end point and let the moveHandle move it for us.
        $(mouseEventData.element).off('CornerstoneToolsMouseMove', cornerstoneTools.length.mouseMoveCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDown', cornerstoneTools.length.mouseDownCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDownActivate', cornerstoneTools.length.mouseDownActivateCallback);
        ...
        cornerstoneTools.moveNewHandle(mouseEventData, toolType, measurementData, measurementData.handles.end, function () {
            ...
            $(mouseEventData.element).on('CornerstoneToolsMouseMove', eventData, cornerstoneTools.length.mouseMoveCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDown', eventData, cornerstoneTools.length.mouseDownCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDownActivate', eventData, cornerstoneTools.length.mouseDownActivateCallback);
            cornerstone.updateImage(mouseEventData.element);
        });
    }
    ...
    //    cornerstoneTools.addToolState(element, toolType, measurementData);
    //    $(element).off('CornerstoneToolsTouchStartActive', cornerstoneTools.lengthTouch.touchDownActivateCallback);
    //    $(element).off('CornerstoneToolsTap', cornerstoneTools.lengthTouch.tapCallback);
    //    cornerstone.updateImage(element);
    ...
    //        $(element).on('CornerstoneToolsTouchStartActive', cornerstoneTools.lengthTouch.touchDownActivateCallback);
    //        $(element).on('CornerstoneToolsTap', cornerstoneTools.lengthTouch.tapCallback);
    //    });
    //}
    function addNewMeasurementTouch(touchEventData) {
        var element = touchEventData.element;
    ...
    function onImageRendered(e, eventData) {
        ...
        //    for (var vd = 0 ; vd < toolData.data.length ; vd++) {
        //        //toolData.data[vd].handles.start.x += 2
        //        //toolData.data[vd].handles.start.y += 2
        //        //toolData.data[vd].handles.end.x += 2
        //        //toolData.data[vd].handles.end.y += 2

        //        //toolData.data[vd].handles.start.x += toolData.data[vd].handles.start.y
        //        //toolData.data[vd].handles.start.y += toolData.data[vd].handles.start.x
        //        //toolData.data[vd].handles.end.x += toolData.data[vd].handles.end.y
        //        //toolData.data[vd].handles.end.y += toolData.data[vd].handles.end.x

        //        toolData.data[vd].handles.start.x += toolData.data[vd].handles.start.x/2
        //        toolData.data[vd].handles.start.y += toolData.data[vd].handles.start.y/2
        //        toolData.data[vd].handles.end.x += toolData.data[vd].handles.end.x/2
        //        toolData.data[vd].handles.end.y += toolData.data[vd].handles.end.y/2
        //    }
        //}        
    ...
    function pressCallback(e, eventData) {
        ...
        function doneChangingTextCallback(data, updatedText, deleteTool) {
            ...
            data.active = false;
            cornerstone.updateImage(element);
            $(element).on('CornerstoneToolsTouchStart', cornerstoneTools.lengthTouch.touchStartCallback);
            $(element).on('CornerstoneToolsTouchStartActive', cornerstoneTools.lengthTouch.touchDownActivateCallback);
        }
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            if (pointNearTool(element, data, coords)) {
                ...
                cornerstone.updateImage(element);
                // Allow relabelling via a callback
                $(element).off('CornerstoneToolsTouchStart', cornerstoneTools.lengthTouch.touchStartCallback);
                $(element).off('CornerstoneToolsTouchStartActive', cornerstoneTools.lengthTouch.touchDownActivateCallback);
                config.changeTextCallback(data, doneChangingTextCallback);
    ...
    cornerstoneTools.length.setConfiguration(configuration);
...
(function ($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    ///////// BEGIN ACTIVE TOOL ///////
    //CHANGED AND EDITED BY VIVEK PATEL
    ...
    function addNewMeasurement(mouseEventData) {
        ...
        // since we are dragging to another place to drop the end point, we can just activate
        // the end point and let the moveHandle move it for us.
        $(mouseEventData.element).off('CornerstoneToolsMouseMove', cornerstoneTools.rectangleRoi.mouseMoveCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDown', cornerstoneTools.rectangleRoi.mouseDownCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDownActivate', cornerstoneTools.rectangleRoi.mouseDownActivateCallback);
        ...
        cornerstoneTools.moveNewHandle(mouseEventData, toolType, measurementData, measurementData.handles.end, function () {
            ...
            $(mouseEventData.element).on('CornerstoneToolsMouseMove', eventData, cornerstoneTools.rectangleRoi.mouseMoveCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDown', eventData, cornerstoneTools.rectangleRoi.mouseDownCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDownActivate', eventData, cornerstoneTools.rectangleRoi.mouseDownActivateCallback);
            cornerstone.updateImage(mouseEventData.element);
        });
    }
    ...
    function pressCallback(e, eventData) {
        ...
        function doneChangingTextCallback(data, updatedText, deleteTool) {
            ...
            data.active = false;
            cornerstone.updateImage(element);
            $(element).on('CornerstoneToolsTouchStart', cornerstoneTools.rectangleRoiTouch.touchStartCallback);
            $(element).on('CornerstoneToolsTouchStartActive', cornerstoneTools.rectangleRoiTouch.touchDownActivateCallback);
        }
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            if (pointNearTool(element, data, coords)) {
                ...
                cornerstone.updateImage(element);
                // Allow relabelling via a callback
                $(element).off('CornerstoneToolsTouchStart', cornerstoneTools.rectangleRoiTouch.touchStartCallback);
                $(element).off('CornerstoneToolsTouchStartActive', cornerstoneTools.rectangleRoiTouch.touchDownActivateCallback);
                config.changeTextCallback(data, doneChangingTextCallback);
    ...
    cornerstoneTools.rectangleRoi.setConfiguration(configuration);
...
(function ($, cornerstone, cornerstoneTools) {
    ...
    function saveAs(element, filename) {
        ...
        var vWorkspaceId, vProjectNo, vSubjectId, iNodeId, iModalityNo, iAnatomyNo, iuserid, iImgTransmittalHdrId, iImgTransmittalDtlId, iBizImageStatus, SaveImageFlag;
        ...
        var v = 0;
        for (var i = 0; i < stack.imageIds.length; i++) {
            ...
            function doneCallback(image) {
                ...
                image.imageId = image.imageId.split("?")[0]
                var canvas = $(element).find('canvas').get(0);

                var DicomImage = canvas.toDataURL("image/png");
                //var DicomImage = canvas.toDataURL("image/jpeg");

                //Pic = Pic.replace(/^data:image\/(png|jpg);base64,/, "")

                DicomImage = DicomImage.replace('data:image/png;base64,', '');
                //DicomImage = DicomImage.replace('data:image/jpeg;base64,', '');


                //img.push(Pic);
                var currentdate = new Date();
                var DicomName = currentdate.getDate() + ""
                ...
                if (query == '' || query == "" || query == null) {
                    var iImgTransmittalHdrId = $("#hdniImgTransmittalHdrId").val();
                    var iImgTransmittalDtlId = $("#hdniImgTransmittalDtlId").val();
                    var vWorkspaceId = $("#hdnvWorkspaceId").val();
                    var vProjectNo = $("#hdnvProjectNo").val();
                    var vSubjectId = $("#hdnvSubjectId").val();
                    var iNodeId = $("#hdniNodeId").val();
                    var iModalityNo = $("#hdniModalityNo").val();
                    var iAnatomyNo = $("#hdniAnatomyNo").val();
                    var cRadiologist

                    cRadiologist = $("#hdnvSubActivityName").val().split('-')[0]

                    if (!cRadiologist.toUpperCase().match("R")) {
                        alert("No Radiologist Detail Found To Save Dicom Detail")
                        return false;
                    }


                    $.ajax({
                        url: WebURL + "MIDicomViewer/UploadDicom",
                        type: 'POST',
                        crossDomain: true,
                        data: '{ "DicomImage" : "' + DicomImage + '" ,"DicomName" : "' + i + '","stackLength":"' + stackLength + '", "vWorkspaceId":"' + vWorkspaceId + '","vProjectNo":"' + vProjectNo + '","vSubjectId":"' + vSubjectId + '","iNodeId":"' + iNodeId + '","iModalityNo":"' + iModalityNo + '","iAnatomyNo":"' + iAnatomyNo + '","iImgTransmittalHdrId":"' + iImgTransmittalHdrId + '","iImgTransmittalDtlId":"' + iImgTransmittalDtlId + '","cRadiologist":"' + cRadiologist + '","ImageTransmittalImgDtl_iImageTranNo":"' + $("#hdnImageTransmittalImgDtl_iImageTranNo").val() + '"}',
                        contentType: 'application/json; charset=utf-8',
                        dataType: 'text',
                        async: false,
                        success: function (data) {
                            var k = stack.imageIds.length;
                            if (data == "success") {
                                if (v == k - 1) {
                                    //alert("11");
                                    MIDicomSave = true;
                                    //AlertBox("SUCCESS", " Dicom Viewer", "Saving Image Successfully.!")
                                }
                            }
                                ...
                                if (v == k - 1) {
                                    ...
                                    MISessionExpired = true;
                                    //MISession = false;
                                }
                            }
                            else if (data == "ErrorNoPathFound") {
                                if (v == k - 1) {
                                    //alert("No Path Found To Save Image");
                                    //alert("33");
                                    AlertBox("ERROR", " Dicom Viewer", "No Path Found To Save Image.!")
                                    MISessionExpired = true;
                                    //MISession = false;
                                }
                            }
                                ...
                                if (v == k - 1) {
                                    //alert("55");
                                    MIDicomSave = false;
                                    //alert("Error While Saving Image Detail")
                                    AlertBox("ERROR", " Dicom Viewer", "Error While Saving Image Detail.!")
                                }
                            }
                            v++;
                        },
                    ...
                    //url: WebURL + "MIDicomViewer/BizNETUploadSaveDicom",
                    //crossDomain: true,
                    //data: MISaveDicom,
                    //data: '{ "DicomImage" : "' + DicomImage + '" ,"DicomName" : "' + i + '","stackLength":"' + stackLength + '","vWorkspaceId": "' + vWorkspaceId + '","vProjectNo":"' + vProjectNo + '","vSubjectId":"' + vSubjectId + '","iNodeId":"' + iNodeId + '","iModalityNo":"' + iModalityNo + '","iAnatomyNo":"' + iAnatomyNo + '","iuserid":"' + iuserid + '","iImgTransmittalHdrId":"' + iImgTransmittalHdrId + '","iImgTransmittalDtlId":"' + iImgTransmittalDtlId + '"}',
                    //contentType: 'application/json; charset=utf-8',
                    //dataType: 'json',
                    //var v = WebURL + "MIDicomViewer/BizNETUploadSaveDicom1";

                    //$.ajax({
                    //    //url: ApiURL + "SetData/test",
                    //    url: WebURL + "MIDicomViewer/BizNETUploadSaveDicom",
                    //    type: 'POST',
                    //    data: MISaveDicom,
                    //    async: false,
                    ...
                    $.ajax({
                        //url: WebURL + "MIDicomViewer/BizNETUploadSaveDicom2",
                        url: WebURL + "MIDicomViewer/BizNETUploadSaveDicom2",
                        type: 'POST',
                        crossDomain: true,
                        data: '{ "DicomImage" : "' + DicomImage + '" ,"DicomName" : "' + i + '","stackLength":"' + stackLength + '","vWorkspaceId": "' + vWorkspaceId + '","vProjectNo":"' + vProjectNo + '","vSubjectId":"' + vSubjectId + '","iNodeId":"' + iNodeId + '","iModalityNo":"' + iModalityNo + '","iAnatomyNo":"' + iAnatomyNo + '","iuserid":"' + iuserid + '","iImgTransmittalHdrId":"' + iImgTransmittalHdrId + '","iImgTransmittalDtlId":"' + iImgTransmittalDtlId + '"}',
                        ...
                        success: function (msg) {
                            ...
                            if (msg == 0) {
                                if (v == k - 1) {
                                    BizNetDicomPath = vWorkspaceId + "_" + vSubjectId + "_" + iNodeId + "_" + iModalityNo
                                    BizNetDicomSave = false;
                                    //alert("Error While Saving Image Detail")
                                    AlertBox("ERROR", " Dicom Viewer", "Error While Saving Image Detail.!")
                                }
                            }
                                ...
                                if (v == k - 1) {
                                    BizNetDicomPath = vWorkspaceId + "_" + vSubjectId + "_" + iNodeId + "_" + iModalityNo
                                    BizNetDicomSave = true;
                                    //alert('Image saved successfully !');
                                    AlertBox("ERROR", " Dicom Viewer", "Image saved successfully.!")
                                }
                            }
                            v++;
                        },
                        error: function (msg) {
                            ...
                            if (v == k - 1) {
                                BizNetDicomPath = vWorkspaceId + "_" + vSubjectId + "_" + iNodeId + "_" + iModalityNo
                                BizNetDicomSave = false;
                                //alert("Error While Saving Image Detail " + msg.responseText)
                                AlertBox("ERROR", " Dicom Viewer", "Error While Saving Image Detail.!" + msg.responseText)
                            }
                            v++;
                        }
                    });
            ...
            //var eventData = {
            //    newImageIdIndex: i,
            //    direction: i - stackData.currentImageIdIndex
            //};

            stackData.currentImageIdIndex = i;

            var newImageId = stackData.imageIds[i];
        ...
        Object.keys(handles).forEach(function (name) {
            ...
            if (handle.hasOwnProperty('pointNearHandle')) {
                ...
                if (distance <= distanceThreshold) {
                    nearbyHandle = handle;
                    if (handle.hasOwnProperty('ToolName')) {
                        handles.orthoStart.locked = false;
                    }
                    return;
                }
            }
...
(function ($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    //START UPDATE ORTHO Ruler DATA
    //--- Added by Vijay Rathod ,Used in OrthoRuler 
    ...
    function setHandlesPosition(handle, eventData, data, distanceFromTool) {
        var movedPoint;
        var outOfBounds;
        var result;
        var intersection;
        ...
        if (handle.index === 0) {
            ...
            perpendicularLine.start = {
                x: data.handles.orthoEnd.x,
                y: data.handles.orthoEnd.y,
            };
            ...
            if (!intersection) {
                perpendicularLine.end = {
                    x: data.handles.orthoStart.x,
                    y: data.handles.orthoStart.y,
                };

                intersection = cornerstoneMath.lineSegment.intersectLine(longLine, perpendicularLine);

                d1 = cornerstoneMath.point.distance(
                 intersection,
                 data.handles.start
               );
                d2 = cornerstoneMath.point.distance(
                  intersection,
                  data.handles.end
                );
            ...
            if (!outOfBounds) {
                movedPoint = movePerpendicularLine(
                  ...
                  data,
                  eventData,
                  data.handles.orthoEnd
                );

                if (!movedPoint) {
                    eventData.currentPoints.image.x = data.handles.orthoStart.x;
                    eventData.currentPoints.image.y = data.handles.orthoStart.y;
                }
            }
            ...
            intersection = cornerstoneMath.lineSegment.intersectLine(
              longLine,
              perpendicularLine
            );
            if (!intersection) {
            ...
            if (!outOfBounds) {
                movedPoint = movePerpendicularLine(
                  ...
                  data,
                  eventData,
                  data.handles.orthoStart
                );

                if (!movedPoint) {
                    eventData.currentPoints.image.x = data.handles.orthoEnd.x;
                    eventData.currentPoints.image.y = data.handles.orthoEnd.y;
                }
            }
        }
    }
    ...
    function moveLongLine(proposedPoint, measurementData, eventData, fixedPoint) {
        var baseData = getBaseData(measurementData, eventData, fixedPoint);
        var columnPixelSpacing = baseData.columnPixelSpacing;
        var rowPixelSpacing = baseData.rowPixelSpacing;
        var distanceToFixed = baseData.distanceToFixed;

        // Calculate the length of the new line, considering the proposed point
        var newLineLength = getDistanceWithPixelSpacing(
          columnPixelSpacing,
          rowPixelSpacing,
          fixedPoint,
          proposedPoint
        ...
        var newLine = updatePerpendicularLineForLongLine(baseData, newIntersection);

        // Update the perpendicular line handles
        measurementData.handles.orthoStart.x = newLine.start.x;
        measurementData.handles.orthoStart.y = newLine.start.y;
        measurementData.handles.orthoEnd.x = newLine.end.x;
        measurementData.handles.orthoEnd.y = newLine.end.y;
    ...
    function getBaseData(measurementData, eventData, fixedPoint) {
        var lineSegment = cornerstoneMath.lineSegment;
        var start = measurementData.handles.start;
        var end = measurementData.handles.end;
        var orthoStart = measurementData.handles.orthoStart;
        var orthoEnd = measurementData.handles.orthoEnd;

        var columnPixelSpacing = eventData.image.columnPixelSpacing;
        var rowPixelSpacing = eventData.image.rowPixelSpacing;

        var longLine = createLine(start, end);
        var perpendicularLine = createLine(orthoStart, orthoEnd);
        var intersection = lineSegment.intersectLine(longLine, perpendicularLine);

        var distanceToFixed = getDistanceWithPixelSpacing(
           columnPixelSpacing,
           rowPixelSpacing,
           fixedPoint,
           intersection
        ...
        var Basedata = { columnPixelSpacing: columnPixelSpacing, rowPixelSpacing: rowPixelSpacing, start: start, end: end, orthoStart: orthoStart, orthoEnd: orthoEnd, longLine: longLine, intersection: intersection, distanceToFixed: distanceToFixed, fixedPoint: fixedPoint };
        return Basedata;
    }
    ...
    function updatePerpendicularLineForLongLine(baseData, mid) {

        var columnPixelSpacing = baseData.columnPixelSpacing;
        var rowPixelSpacing = baseData.rowPixelSpacing;
        var start = baseData.start;
        var orthoStart = baseData.orthoStart;
        var orthoEnd = baseData.orthoEnd;
        var intersection = baseData.intersection;
        var fixedPoint = baseData.fixedPoint;


        // Get the original distance from perpendicular start handle to intersection
        var distancePS = getDistanceWithPixelSpacing(
          columnPixelSpacing,
          rowPixelSpacing,
          orthoStart,
          intersection
        ...
        var distancePE = getDistanceWithPixelSpacing(
          columnPixelSpacing,
          rowPixelSpacing,
          orthoEnd,
          intersection
        ...
        var columnMultiplier = multiplier * columnPixelSpacing;
    ...
    function updatePerpendicularLine(baseData, mid, helperLine, vector) {
        ...
        // Get the multiplier
        var multiplier = getDirectionMultiplier(fixedPoint, orthoEnd) * distanceToFixed;

        // Define the moving point
        var movingPoint = getMovingPoint(
          fixedPoint,
          orthoStart,
          orthoEnd
        );
        ...
        var isMovingStart = movingPoint === orthoStart;
        var movingKey = isMovingStart ? 'start' : 'end';
        var fixedKey = isMovingStart ? 'end' : 'start';
    ...
    function getDirectionMultiplier(fixedPoint, orthoEnd) {
        return isPerpendicularEndFixed(fixedPoint, orthoEnd) ? -1 : 1;
    }

    function isPerpendicularEndFixed(fixedPoint, orthoEnd) {
        return fixedPoint === orthoEnd;
    }

    function getMovingPoint(fixedPoint, orthoStart, orthoEnd) {
        if (isPerpendicularEndFixed(fixedPoint, orthoEnd)) {
            return orthoStart;
        }
    ...
    function movePerpendicularLine(proposedPoint, measurementData, eventData, fixedPoint) {
        ...
        var rowPixelSpacing = baseData.rowPixelSpacing;
        var start = baseData.start;
        var longLine = baseData.longLine;
        var intersection = baseData.intersection;
        ...
        // Inclination of the perpendicular line
        var vector = getLineVector(columnPixelSpacing, rowPixelSpacing, start, intersection);

        // Get a helper line to calculate the intersection
        var helperLine = getHelperLine(baseData, proposedPoint, vector);

        // Find the new intersection in the long line
        var newIntersection = lineSegment.intersectLine(longLine, helperLine);
        ...
        // Calculate and the new position of the perpendicular handles
        var newLine = updatePerpendicularLine(baseData, newIntersection, helperLine, vector);
    ...
    function lineHasLength(columnPixelSpacing, rowPixelSpacing, line) {
        var lineLength = getDistanceWithPixelSpacing(
          columnPixelSpacing,
          rowPixelSpacing,
          line.start,
          line.end
    ...
    function getHelperLine(baseData, proposedPoint, vector) {
        ...
        // Get the multiplier
        var multiplier =
          getDirectionMultiplier(fixedPoint, orthoEnd) * highNumber;
...
(function ($, cornerstone, cornerstoneTools) {
    ...
    var scrollBarHeight = 1; //Chage the Thickness of ScorllBar
...
(function (cornerstone, cornerstoneTools) {
    ...
    function newImageIdSpecificToolStateManager() {
        ...
        function addImageIdSpecificToolState(element, toolType, data) {
            ...
            var AllToolData = toolState[enabledImage.image.imageId];
            // finally, add this new tool to the state
            toolData.data.push(data);
        }
...
(function (cornerstoneTools) {
    ...
    function calculateSUV(image, storedPixelValue) {
        ...
        //var patientWeight = image.data.floatString('x00101030'); // in kg //Commented by Bhargav Thaker
        var patientWeight = parseFloat(image.data.string('x00101030', 0)) // in kg //Added by Bhargav Thaker
        if (patientWeight === undefined) { return undefined; }

        var petSequence = image.data.elements.x00540016;
        if (petSequence === undefined) { return undefined; }

        petSequence = petSequence.items[0].dataSet;
        //var startTime = petSequence.time('x00181072'); //Commented by Bhargav Thaker
        //var totalDose = petSequence.floatString('x00181074'); //Commented by Bhargav Thaker
        //var halfLife = petSequence.floatString('x00181075'); //Commented by Bhargav Thaker
        //var acquisitionTime = image.data.time('x00080032'); //Commented by Bhargav Thaker
        var startTime = petSequence.string('x00181072', 0); //Added by Bhargav Thaker
        var STHours = parseInt(startTime.substring(0, 2), 10); //Added by Bhargav Thaker
        var STMinutes = startTime.length >= 4 ? parseInt(startTime.substring(2, 4), 10) : void 0; //Added by Bhargav Thaker
        var STSeconds = startTime.length >= 6 ? parseInt(startTime.substring(4, 6), 10) : void 0; //Added by Bhargav Thaker
        var STfractionalSeconds = startTime.length >= 8 ? parseInt(startTime.substring(7, 13), 10) : void 0; //Added by Bhargav Thaker
        var totalDose = parseFloat(petSequence.string('x00181074', 0)); //Added by Bhargav Thaker
        var halfLife = parseFloat(petSequence.string('x00181075', 0)); //Added by Bhargav Thaker
        var acquisitionTime = image.data.string('x00080032', 0); //Added by Bhargav Thaker
        var ATHours = parseInt(acquisitionTime.substring(0, 2), 10); //Added by Bhargav Thaker
        var ATMinutes = acquisitionTime.length >= 4 ? parseInt(acquisitionTime.substring(2, 4), 10) : void 0; //Added by Bhargav Thaker
        var ATSeconds = acquisitionTime.length >= 6 ? parseInt(acquisitionTime.substring(4, 6), 10) : void 0; //Added by Bhargav Thaker
        var ATfractionalSeconds = acquisitionTime.length >= 8 ? parseInt(acquisitionTime.substring(7, 13), 10) : void 0; //Added by Bhargav Thaker

        if (!startTime || !totalDose || !halfLife || !acquisitionTime) {
            return undefined;
        }

        //var acquisitionTimeInSeconds = acquisitionTime.fractionalSeconds + acquisitionTime.seconds + acquisitionTime.minutes * 60 + acquisitionTime.hours * 60 * 60; //Commented by Bhargav Thaker
        //var injectionStartTimeInSeconds = startTime.fractionalSeconds + startTime.seconds + startTime.minutes * 60 + startTime.hours * 60 * 60; //Commented by Bhargav Thaker
        var acquisitionTimeInSeconds = ATfractionalSeconds + ATSeconds + ATMinutes * 60 + ATHours * 60 * 60; //Added by Bhargav Thaker
        var injectionStartTimeInSeconds = STfractionalSeconds + STSeconds + STMinutes * 60 + STHours * 60 * 60; //Added by Bhargav Thaker
...
(function (cornerstoneTools) {
    ...
    function drawTextBox(context, textLines, x, y, color) {
        ...
        textLines.forEach(function (text, index) {
            ...
            //$('.25041').each(function (index, control) {
            //    if (control.length != 0) {
...
// Begin : Save DicomAnnotation

(function (cornerstone, cornerstoneTools) {
    function saveDicomAnnotation() {
        debugger;
        var DicomDataAnnotation = [];
        for (var i = 0; i < stack.imageIds.length; i++) {

            var newImageIdIndex = i;
            ...
            DicomDataAnnotation = [].concat(DicomDataAnnotation, getImagewiseToolData(dicomFileName, dicomServerPath));
        }
        return DicomDataAnnotation;
    };
    function getImagewiseToolData(dicomFileName, dicomServerPath, dicomStatusIndi) {

        var DicomDataAnnotation = [], toolData;

        toolData = cornerstoneTools.getToolState(element, 'length');
        if (toolData !== undefined) {

            for (var vd = 0 ; vd < toolData.data.length ; vd++) {
                DataAnnotation = {
                    vFileName: dicomFileName,
                    vServerPath: dicomServerPath,
                    cStatusIndi: dicomStatusIndi,
                    vAnnotationType: 'length',
                    nvDicomAnnotation: JSON.stringify(toolData.data[vd])
                }
                DicomDataAnnotation.push(DataAnnotation);
            }
        }
        ...
        toolData = cornerstoneTools.getToolState(element, 'ortho');
        if (toolData !== undefined) {
            for (var vd = 0 ; vd < toolData.data.length ; vd++) {
                DataAnnotation = {
                    vFileName: dicomFileName,
        ...
        toolData = cornerstoneTools.getToolState(element, 'perpendicular');
        if (toolData !== undefined) {
            for (var vd = 0 ; vd < toolData.data.length ; vd++) {
                DataAnnotation = {
                    vFileName: dicomFileName,
        ...
        //--- Added by Vijay Rathod , For save ellipticalroi Tools data
        toolData = cornerstoneTools.getToolState(element, 'ellipticalRoi');
        if (toolData !== undefined) {
            for (var vd = 0 ; vd < toolData.data.length ; vd++) {
                DataAnnotation = {
                    vFileName: dicomFileName,
    ...
    cornerstoneTools.getImagewiseToolData = getImagewiseToolData;
    cornerstoneTools.saveDicomAnnotation = saveDicomAnnotation;
})(cornerstone, cornerstoneTools);

// End : Save DicomAnnotation

// Begin : Ortho Perpedicular Tool

//Tool with Simple Angle Logic For Orthogonal ruler
//(function ($, cornerstone, cornerstoneMath, cornerstoneTools) {
...
//        // since we are dragging to another place to drop the end point, we can just activate
//        // the end point and let the moveHandle move it for us.
//        $(element).off('CornerstoneToolsMouseMove', cornerstoneTools.ortho.mouseMoveCallback);
//        $(element).off('CornerstoneToolsMouseDrag', cornerstoneTools.ortho.mouseMoveCallback);
//        $(element).off('CornerstoneToolsMouseDown', cornerstoneTools.ortho.mouseDownCallback);
//        //$(element).off('CornerstoneToolsMouseDownActivate', cornerstoneTools.ortho.mouseDownActivateCallback);
//        cornerstone.updateImage(element);
...
//                $(element).on('CornerstoneToolsMouseMove', cornerstoneTools.ortho.mouseMoveCallback);
//                $(element).on('CornerstoneToolsMouseDrag', cornerstoneTools.ortho.mouseMoveCallback);
//                $(element).on('CornerstoneToolsMouseDown', eventData, cornerstoneTools.ortho.mouseDownCallback);
//                //$(element).on('CornerstoneToolsMouseDownActivate', eventData, cornerstoneTools.ortho.mouseDownActivateCallback);
//                cornerstone.updateImage(element);
//            });
//        });
//    }
...
//    ///////// END ACTIVE TOOL ///////
//    cornerstoneTools.ortho = cornerstoneTools.mouseButtonTool({
//        createNewMeasurement: createNewMeasurement,
//        addNewMeasurement: addNewMeasurement,
//        onImageRendered: onImageRendered,
//        pointNearTool: pointNearTool,
...
//Tool with Line Logic For Orthogonal ruler
(function ($, cornerstone, cornerstoneMath, cornerstoneTools) {
    ...
    function addNewMeasurement(mouseEventData) {
        ...
        // since we are dragging to another place to drop the end point, we can just activate
        // the end point and let the moveHandle move it for us.
        $(mouseEventData.element).off('CornerstoneToolsMouseMove', cornerstoneTools.ortho.mouseMoveCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDown', cornerstoneTools.ortho.mouseDownCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDownActivate', cornerstoneTools.ortho.mouseDownActivateCallback);
        ...
        cornerstoneTools.moveNewHandle(mouseEventData, toolType, measurementData, measurementData.handles.end, function () {
            ...
            $(mouseEventData.element).on('CornerstoneToolsMouseMove', eventData, cornerstoneTools.ortho.mouseMoveCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDown', eventData, cornerstoneTools.ortho.mouseDownCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDownActivate', eventData, cornerstoneTools.ortho.mouseDownActivateCallback);
            cornerstone.updateImage(mouseEventData.element);
        });
    }
    ...
    function addNewMeasurementTouch(touchEventData) {
        ...
        cornerstoneTools.addToolState(element, toolType, measurementData);
        $(element).off('CornerstoneToolsTouchStartActive', cornerstoneTools.orthoTouch.touchDownActivateCallback);
        $(element).off('CornerstoneToolsTap', cornerstoneTools.orthoTouch.tapCallback);
        cornerstone.updateImage(element);
        ...
        cornerstoneTools.moveNewHandleTouch(touchEventData, toolType, measurementData, measurementData.handles.end, function () {
            ...
            $(element).on('CornerstoneToolsTouchStartActive', cornerstoneTools.orthoTouch.touchDownActivateCallback);
            $(element).on('CornerstoneToolsTap', cornerstoneTools.orthoTouch.tapCallback);
        });
    }
    ...
    function pointNearTool(element, data, coords) {
        var lineSegment = {
            ...
            //start: cornerstone.pixelToCanvas(element, data.handles.orthoStart),
            //end: cornerstone.pixelToCanvas(element, data.handles.orthoEnd)
        };
        ...
        var distanceToPoint = cornerstoneMath.lineSegment.distanceToPoint(lineSegment, coords);
        var distanceToPoint1 = cornerstoneMath.lineSegment.distanceToPoint(lineSegment1, coords);

        return (distanceToPoint < 25 || distanceToPoint1 < 25);
    }
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            //--- Changed logic by Vijay Rathod , For OrthoRuler 
            //Perpendicular Angle            
    ...
    /////BEGIN UPDATE ORTHO RULER DATA////
    function updatePerpendicularLineHandles(eventData, measurementData) {
        if (!measurementData.handles.orthoStart.locked) {
            return false;
        }
        ...
        if (YAxisData.start.x === YAxisData.end.x && YAxisData.start.y === YAxisData.end.y) {
            startX = YAxisData.start.x;
            startY = YAxisData.start.y;
            endX = YAxisData.end.x;
            endY = YAxisData.end.y;
        } else {
            // Mid point of long-axis line
            var mid = {
                x: (YAxisData.start.x + YAxisData.end.x) / 2,
                y: (YAxisData.start.y + YAxisData.end.y) / 2,
            };
        ...
        measurementData.handles.orthoStart.x = startX;
        measurementData.handles.orthoStart.y = startY;
        measurementData.handles.orthoEnd.x = endX;
        measurementData.handles.orthoEnd.y = endY;
    ...
    function pressCallback(e, eventData) {
        ...
        function doneChangingTextCallback(data, updatedText, deleteTool) {
            ...
            data.active = false;
            cornerstone.updateImage(element);
            $(element).on('CornerstoneToolsTouchStart', cornerstoneTools.orthoTouch.touchStartCallback);
            $(element).on('CornerstoneToolsTouchStartActive', cornerstoneTools.orthoTouch.touchDownActivateCallback);
        }
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            if (pointNearTool(element, data, coords)) {
                ...
                cornerstone.updateImage(element);
                // Allow relabelling via a callback
                $(element).off('CornerstoneToolsTouchStart', cornerstoneTools.orthoTouch.touchStartCallback);
                $(element).off('CornerstoneToolsTouchStartActive', cornerstoneTools.orthoTouch.touchDownActivateCallback);
                config.changeTextCallback(data, doneChangingTextCallback);
    ...
    cornerstoneTools.ortho.setConfiguration(configuration);

    cornerstoneTools.orthoTouch = cornerstoneTools.touchTool({
        addNewMeasurementTouch: addNewMeasurementTouch,
        createNewMeasurement: createNewMeasurement,
        onImageRendered: onImageRendered,
        pointNearTool: pointNearTool,
...
// End : Ortho Perpedicular Tool

// Begin : Perpedicular Ortho Tool

(function ($, cornerstone, cornerstoneMath, cornerstoneTools) {

    'use strict';

    var toolType = 'perpendicular';
    ...
    function getTextCallback(doneChangingTextCallback) {
        ...
        console.log(opendialog + "  " + isIntersect);
        count = count + 1;
        if (isIntersect == true && opendialog == true) {
    ...
    function addNewMeasurement(mouseEventData) {
        ...
        // since we are dragging to another place to drop the end point, we can just activate
        // the end point and let the moveHandle move it for us.
        $(mouseEventData.element).off('CornerstoneToolsMouseMove', cornerstoneTools.perpendicular.mouseMoveCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDown', cornerstoneTools.perpendicular.mouseDownCallback);
        $(mouseEventData.element).off('CornerstoneToolsMouseDownActivate', cornerstoneTools.perpendicular.mouseDownActivateCallback);
        ...
        cornerstoneTools.moveNewHandle(mouseEventData, toolType, measurementData, measurementData.handles.end, function () {
            ...
            var config = cornerstoneTools.perpendicular.getConfiguration();
            //var openDialog = false;
            //if ((count > 0 && (count / 2) == 0) || count==0)
            //{
            //    openDialog = true;
            ...
            //if (isIntersect == true && openDialog) {
            ...
            $(mouseEventData.element).on('CornerstoneToolsMouseMove', eventData, cornerstoneTools.perpendicular.mouseMoveCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDown', eventData, cornerstoneTools.perpendicular.mouseDownCallback);
            $(mouseEventData.element).on('CornerstoneToolsMouseDownActivate', eventData, cornerstoneTools.perpendicular.mouseDownActivateCallback);
            cornerstone.updateImage(mouseEventData.element);
        });
    }
    ...
    function addNewMeasurementTouch(touchEventData) {
        ...
        cornerstoneTools.addToolState(element, toolType, measurementData);
        $(element).off('CornerstoneToolsTouchStartActive', cornerstoneTools.perpendicularTouch.touchDownActivateCallback);
        $(element).off('CornerstoneToolsTap', cornerstoneTools.perpendicularTouch.tapCallback);
        cornerstone.updateImage(element);
        ...
        cornerstoneTools.moveNewHandleTouch(touchEventData, toolType, measurementData, measurementData.handles.end, function () {
            ...
            $(element).on('CornerstoneToolsTouchStartActive', cornerstoneTools.perpendicularTouch.touchDownActivateCallback);
            $(element).on('CornerstoneToolsTap', cornerstoneTools.perpendicularTouch.tapCallback);
        });
    }
    ...
    function onImageRendered(e, eventData) {
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            if (j % 2 == 0) {
                ...
                //debugger
                if (checkIntersection(toolData.data[i - 1].handles.start.x, toolData.data[i - 1].handles.start.y, toolData.data[i - 1].handles.end.x, toolData.data[i - 1].handles.end.y,
                        toolData.data[i].handles.start.x, toolData.data[i].handles.start.y, toolData.data[i].handles.end.x, toolData.data[i].handles.end.y)) {
                    ...
                    //}
                    //alert("intersect");
                    //var results = checkLineIntersection(toolData.data[i-1].handles.start.x, toolData.data[i-1].handles.start.y, toolData.data[i-1].handles.end.x, toolData.data[i-1].handles.end.y,
                    //    toolData.data[i].handles.start.x, toolData.data[i].handles.start.y, toolData.data[i].handles.end.x, toolData.data[i ].handles.end.y)                    

                    toolData.data[i].handles.start.perpendicular = true
                    toolData.data[i].handles.end.perpendicular = true

                    var line1StartX = toolData.data[i - 1].handles.start.x;
                    var line1StartY = toolData.data[i - 1].handles.start.y;
                    var line1EndX = toolData.data[i - 1].handles.end.x;
                    var line1EndY = toolData.data[i - 1].handles.end.y;
                    var line2StartX = toolData.data[i].handles.start.x;
                    var line2StartY = toolData.data[i].handles.start.y;
                    var line2EndX = toolData.data[i].handles.end.x;
                    var line2EndY = toolData.data[i].handles.end.y;
                    ...
                    var sideA = {
                        x: (Math.ceil(result.x) - Math.ceil(toolData.data[i - 1].handles.start.x)) * columnPixelSpacing,
                        y: (Math.ceil(result.y) - Math.ceil(toolData.data[i - 1].handles.start.y)) * rowPixelSpacing
                    };

                    var sideB = {
                        x: (Math.ceil(toolData.data[i].handles.end.x) - Math.ceil(result.x)) * columnPixelSpacing,
                        y: (Math.ceil(toolData.data[i].handles.end.y) - Math.ceil(result.y)) * rowPixelSpacing
                    };

                    var sideC = {
                        x: (Math.ceil(toolData.data[i].handles.end.x) - Math.ceil(toolData.data[i - 1].handles.start.x)) * columnPixelSpacing,
                        y: (Math.ceil(toolData.data[i].handles.end.y) - Math.ceil(toolData.data[i - 1].handles.start.y)) * rowPixelSpacing
                    };
                    ...
                    var sideALength = Math.sqrt(Math.pow(sideA.x, 2) + Math.pow(sideA.y, 2));
                    var sideBLength = Math.sqrt(Math.pow(sideB.x, 2) + Math.pow(sideB.y, 2))
                    var sideCLength = Math.sqrt(Math.pow(sideC.x, 2) + Math.pow(sideC.y, 2))
                    ...
                    //var rAngle = cornerstoneTools.roundToDecimal(angle, 2);
                    var rAngle = cornerstoneTools.roundToDecimal(angle, 0);
                    ...
                    if (rAngle) {
                        ...
                        var handleStartCanvas = cornerstone.pixelToCanvas(eventData.element, toolData.data[i - 1].handles.start);
                        var handleMiddleCanvas = cornerstone.pixelToCanvas(eventData.element, result);
                        var handleEndCanvas = cornerstone.pixelToCanvas(eventData.element, toolData.data[i].handles.end);
            ...
            cornerstoneTools.drawTextBox(newContext, newText, newX, newY, newColor);
            //cornerstoneTools.drawTextBox(context, text, textCoords.x, textCoords.y, color);
        }
    }
    ///////// END IMAGE RENDERING ///////
    ...
    function pressCallback(e, eventData) {
        ...
        function doneChangingTextCallback(data, updatedText, deleteTool) {
            ...
            data.active = false;
            cornerstone.updateImage(element);
            $(element).on('CornerstoneToolsTouchStart', cornerstoneTools.perpendicularTouch.touchStartCallback);
            $(element).on('CornerstoneToolsTouchStartActive', cornerstoneTools.perpendicularTouch.touchDownActivateCallback);
        }
        ...
        for (var i = 0; i < toolData.data.length; i++) {
            ...
            if (pointNearTool(element, data, coords)) {
                ...
                cornerstone.updateImage(element);
                // Allow relabelling via a callback
                $(element).off('CornerstoneToolsTouchStart', cornerstoneTools.perpendicularTouch.touchStartCallback);
                $(element).off('CornerstoneToolsTouchStartActive', cornerstoneTools.perpendicularTouch.touchDownActivateCallback);
                config.changeTextCallback(data, doneChangingTextCallback);
    ...
    // module exports
    cornerstoneTools.perpendicular = cornerstoneTools.mouseButtonTool({
        addNewMeasurement: addNewMeasurement,
        createNewMeasurement: createNewMeasurement,
        onImageRendered: onImageRendered,
        pointNearTool: pointNearTool,
    ...
    cornerstoneTools.perpendicular.setConfiguration(configuration);

    cornerstoneTools.perpendicularTouch = cornerstoneTools.touchTool({
        addNewMeasurementTouch: addNewMeasurementTouch,
        createNewMeasurement: createNewMeasurement,
        onImageRendered: onImageRendered,
        pointNearTool: pointNearTool,
...
// End : Perpedicular Ortho Tool

// Begin Source: Ct abdomen
(function ($, cornerstone, cornerstoneTools) {
    ...
    function mouseDragCallback(e, eventData) {
        cornerstoneTools.Abdomen.strategy(eventData);
        cornerstone.setViewport(eventData.element, eventData.viewport);
        return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    }
    ...
    function defaultStrategy() {
        ...
        eventData.viewport.voi.windowWidth = 400;
        eventData.viewport.voi.windowCenter = 60;
        cornerstone.setViewport(eventData.element, eventData.viewport);
    }

    cornerstoneTools.Abdomen = cornerstoneTools.simpleMouseButtonTool(mouseDownCallback);
    cornerstoneTools.Abdomen.strategies = {
        default: defaultStrategy
    };

    cornerstoneTools.Abdomen.strategy = defaultStrategy;

})($, cornerstone, cornerstoneTools);

// End Source; Ct abdomen

// Begin Source: CT Angio
(function ($, cornerstone, cornerstoneTools) {
    ...
    function mouseDragCallback(e, eventData) {
        cornerstoneTools.Angio.strategy(eventData);
        cornerstone.setViewport(eventData.element, eventData.viewport);
        return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    }
    ...
    function defaultStrategy() {
        var eventData = cornerstone.getEnabledElement($("#dicomImage")[0]);
        eventData.viewport.voi.windowWidth = 600;
        eventData.viewport.voi.windowCenter = 300;
        cornerstone.setViewport(eventData.element, eventData.viewport);
    }

    cornerstoneTools.Angio = cornerstoneTools.simpleMouseButtonTool(mouseDownCallback);
    cornerstoneTools.Angio.strategies = {
        default: defaultStrategy
    };

    cornerstoneTools.Angio.strategy = defaultStrategy;

})($, cornerstone, cornerstoneTools);

// End Source; CT Angio

// Begin Source: CT Bone
(function ($, cornerstone, cornerstoneTools) {
    ...
    function mouseDragCallback(e, eventData) {
        cornerstoneTools.Bone.strategy(eventData);
        cornerstone.setViewport(eventData.element, eventData.viewport);
        return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    }
    ...
    function defaultStrategy() {
        var eventData = cornerstone.getEnabledElement($("#dicomImage")[0]);
        eventData.viewport.voi.windowWidth = 1500;
        eventData.viewport.voi.windowCenter = 300;
        cornerstone.setViewport(eventData.element, eventData.viewport);
    }

    cornerstoneTools.Bone = cornerstoneTools.simpleMouseButtonTool(mouseDownCallback);
    cornerstoneTools.Bone.strategies = {
        default: defaultStrategy
    };

    cornerstoneTools.Bone.strategy = defaultStrategy;

})($, cornerstone, cornerstoneTools);

// End Source; CT Bone

// Begin Source: CT Brain
(function ($, cornerstone, cornerstoneTools) {
    ...
    function mouseDragCallback(e, eventData) {
        cornerstoneTools.Brain.strategy(eventData);
        cornerstone.setViewport(eventData.element, eventData.viewport);
        return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    }
    ...
    function defaultStrategy() {
        var eventData = cornerstone.getEnabledElement($("#dicomImage")[0]);
        eventData.viewport.voi.windowWidth = 80;
        eventData.viewport.voi.windowCenter = 40;
        cornerstone.setViewport(eventData.element, eventData.viewport);
    }

    cornerstoneTools.Brain = cornerstoneTools.simpleMouseButtonTool(mouseDownCallback);
    cornerstoneTools.Brain.strategies = {
        default: defaultStrategy
    };

    cornerstoneTools.Brain.strategy = defaultStrategy;

})($, cornerstone, cornerstoneTools);

// End Source; CT Brain

// Begin Source: CT Chest
(function ($, cornerstone, cornerstoneTools) {
    ...
    function mouseDragCallback(e, eventData) {
        cornerstoneTools.Chest.strategy(eventData);
        cornerstone.setViewport(eventData.element, eventData.viewport);
        return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    }
    ...
    cornerstoneTools.Chest = cornerstoneTools.simpleMouseButtonTool(mouseDownCallback);
    cornerstoneTools.Chest.strategies = {
        default: defaultStrategy
    };

    cornerstoneTools.Chest.strategy = defaultStrategy;

})($, cornerstone, cornerstoneTools);

// End Source; CT Chest

// Begin Source: CT Lungs
(function ($, cornerstone, cornerstoneTools) {
    ...
    function mouseDragCallback(e, eventData) {
        cornerstoneTools.Lungs.strategy(eventData);
        cornerstone.setViewport(eventData.element, eventData.viewport);
        return false; // false = cases jquery to preventDefault() and stopPropagation() this event
    }
    ...
    function defaultStrategy() {
        var eventData = cornerstone.getEnabledElement($("#dicomImage")[0]);
        eventData.viewport.voi.windowWidth = 1500;
        eventData.viewport.voi.windowCenter = -400;
        cornerstone.setViewport(eventData.element, eventData.viewport);
    }

    cornerstoneTools.Lungs = cornerstoneTools.simpleMouseButtonTool(mouseDownCallback);
    cornerstoneTools.Lungs.strategies = {
        default: defaultStrategy
    };

    cornerstoneTools.Lungs.strategy = defaultStrategy;
