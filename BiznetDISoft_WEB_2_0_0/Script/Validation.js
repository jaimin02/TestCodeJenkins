...
case '1':
        ...
        if (!allValid)
        {
        msgalert("Only Space And Letters Are Allowed.");
        document.all[fieldname].value="";
        document.all[fieldname].focus();
        return (false);
        }
...
case '3':
        ...
        if (!allValid)
        {
        msgalert("Only Special Characters Are Allowed.");
        document.all[fieldname].value="";
        return (false);
        }
        break; 
...
case '4':
        ...
        if (!allValid)
        {
        msgalert("Only Letters And Numeric Characters Are Allowed.");
        document.all[fieldname].value="";
        document.all[fieldname].focus();
        return (false);
        }
...
case '5':
        ...
        if (!allValid)
        {
        msgalert("Only Letters And Special Characters Are Allowed.");
        document.all[fieldname].value="";
        document.all[fieldname].focus();
        return (false);
        }
...
case '9':
        ...
        if (!allValid)
        {
        msgalert("Only Use _ as Special Character");
        document.all[fieldname].value="";
        document.all[fieldname].focus();
        return (false);
        }
...
case '10':
        ...
        if (!allValid)
        {
        msgalert("Please Enter Proper Value");
        document.all[fieldname].value="";
        document.all[fieldname].focus();
        return (false);
        }
...
case '11':
        ...
        if (!allValid)
        {
        msgalert("Special Characters Are Not Allowed. Please Enter Proper Remarks");
        return (false);
        }
        return true;
        break;
...
case '13':
        ...
        if (!allValid)
        {
        msgalert("Only Alphabets Are Allowed.");
        fieldname.value="";
        fieldname.focus();
        return (false);
        }
...
default : msgalert("Oh You Have All Rights " + val);
     }
}  
...
{
    //var AlphaNumericRegEx = /[a-zA-Z]*/;  as not working
    //AlphaNumericRegEx = /^[a-zA-Z]* $/g
    AlphaNumericRegEx  = /^[a-zA-Z-,]+(\s{0,1}[a-zA-Z-, ])*$/
    if (!AlphaNumericRegEx.test(val))
    {
        return false;
    }
...
{    
    ...
    {   
       msgalert('From Date Must be Smaller Then To Date');
        return false;
    }
    else
    {
...
function formatTime(time, t) {
    ...
    if ((m = time.match(re))) {
        ...
        t.value = "";
        msgalert("Please Enter Time in HHMM or HH:MM Format Only.");
    }
    return result;
}
//Added by ketan for character left
function ValidateComments(txt, cntField, maxSize, msg) {
    cntField = document.getElementById(cntField);
    if (txt.value.length > maxSize) {
        alert(msg);
        txt.value = $(txt).val().substring(0, maxSize);
    }
    cntField.innerHTML = maxSize - txt.value.length;
    //validateCharacter(txt);
    return false;
}