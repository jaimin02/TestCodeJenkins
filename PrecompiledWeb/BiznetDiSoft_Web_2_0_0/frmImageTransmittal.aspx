<%@ page title="" language="VB" masterpagefile="~/MasterPage.master" autoeventwireup="false" inherits="frmImageTransmittal, App_Web_xjkmyygy" enableEventValidation="false" theme="StyleBlue" viewStateEncryptionMode="Always" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="CPHLAMBDA" Runat="Server">
    ...
    <script type="text/babel" src="Script/ImageTransmittal.js"></script>
    <contenttemplate>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function HandleAuditTrail(ImgTrHdrId) {
            debugger;
            document.getElementById('AuditTrailDiv').style.display = "block";
            $.ajax({
                type: "post",
                url: "frmImageTransmittal.aspx/AuditTrail",
                data: '{"iImgTransmittalHdrId":"' + ImgTrHdrId + '"}',
                ...
                success: function (response) {
                    ...
                    tempTable = "<tr class='trBorder'><th class ='trBorder'>Sr. No</th>" +
                                "<th class ='trBorder'>Screening Number</th>" + 
                                "<th class ='trBorder'>Visit</th>" +
                    ...
                    for (var i = 0; i < eval(response.d).length; i++) {
                        tempTable += "<tr class ='trBorder'><td class ='trBorder'>" + parseInt(i + 1) + "</td>" +
                            " <td class ='trBorder'>" + eval(response.d)[i].Screening + "</td> " +
                            " <td class ='trBorder'>" + eval(response.d)[i].vNodeDisplayName + "</td>" +
        ...
        function HandleEditQuery(vWorkSpaceId, vSubjectId) {
            debugger;
            document.getElementById('EditQueryDiv').style.display = "block";
            //const EditQueryData = { WorkspaceId: HProjectId.value, SubjectId: ddlSubject.value };

            $.ajax({
                type: "post",
                url: "frmImageTransmittal.aspx/EditQuery",
                data: '{"vWorkSpaceId":"' + HProjectId.value + '","vSubjectId":"' + ddlSubject.value + '"}',
                contentType: "application/json; charset=utf-8",
                datatype: JSON,
                success: function (response) {
                    this.responseData = response.data;
                    var tempTable = "";
                    tempTable = "<tr class='trBorder'><th class ='trBorder'>Sr. No</th>" +
                                "<th class ='trBorder'>ProjectNo</th>" +
                                "<th class ='trBorder'>SubjectId</th>" +
                                "<th class ='trBorder'>Subject No.</th>" +
                                "<th class ='trBorder'>Visit</th>" +
                                "<th class ='trBorder'>Remarks</th>" +
                                "<th class ='trBorder'>Change On(Uploader Date)</th>";

                    for (var i = 0; i < eval(response.d).length; i++) {
                        tempTable += "<tr class ='trBorder'><td class ='trBorder'>" + parseInt(i + 1) + "</td>" +
                            " <td class ='trBorder'>" + eval(response.d)[i].vProjectNo + "</td>" +
                            " <td class ='trBorder'>" + eval(response.d)[i].vSubjectId + "</td>" +
                            " <td class ='trBorder'>" + eval(response.d)[i].vMySubjectNo + "</td>" +
                            " <td class ='trBorder'>" + eval(response.d)[i].vNodeDisplayName + "</td>" +
                            " <td class ='trBorder'>" + eval(response.d)[i].vRemarks + "</td> " +
                            //"<td class ='trBorder'> <center><img alt='submit' style ='cursor: pointer' src='images/Edit2.gif' onclick='HandleEditQuery(" + eval(response.d)[i].nQueryNo + ")'></img></center></td>" +
                            " <td class ='trBorder'>" + eval(response.d)[i].dModifyOn + "</td></tr>"
                    }

                    document.getElementById('EditQueryTable').innerHTML = tempTable;
                },
                failure: function (error) {
                    alert(error._message);
                    return false;
