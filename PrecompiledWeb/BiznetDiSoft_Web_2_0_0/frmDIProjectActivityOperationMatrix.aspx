<%@ page title="" language="VB" masterpagefile="~/MasterPage.master" autoeventwireup="false" inherits="frmDIProjectActivityOperationMatrix, App_Web_xjkmyygy" enableEventValidation="false" theme="StyleBlue" viewStateEncryptionMode="Always" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="CPHLAMBDA" runat="Server">
                            ...
                            <div id="divProjectActivityOperationMatrix" style="/*border: 1px solid green; overflow: scroll; width: 100% */">
                                <table style="width: 100%" cellpadding="5px">
                                    ...
                                    <tr align="left">
                                        ...
                                        <td colspan="3" valign="top" style="width: 60%; display: none;">
                                            <fieldset id="divSubject" runat="server" class="FieldSetBox" style="overflow: auto; display: none; max-height: 230px; height: 230px"
                                                tabindex="7">
                                                <asp:TreeView ID="tvSubject" runat="server" BorderColor="DarkGreen" Font-Bold="True"
                                                    Font-Size="X-Small" Height="250px" ShowCheckBoxes="All" Width="100px">
                                                </asp:TreeView>
                                    ...
                                    <tr>
                                        <td style="text-align: center;" class="Label" colspan="100%">
                                            ...
                                            <asp:Button ID="btnExporttoPdf" runat="server"
                                                    CssClass="btn btnpdf" ToolTip="Export To PDF" OnClientClick="return exportPDF();"/>
                                        </td>
                                    </tr>
                                </table>
                            </div>
                            ...
                            <div id="legend" style="float: right; border: inset; width: 13%;">
                                ...
                                <div>
                                    <fieldset style="display: none; width: 85%; font-size: 7pt; height: auto; text-align: left; margin-left: 3%;"
                                        ...
                                        <div>
                                            <span>
                                                <canvas id="Canvas1" width="12" height="12" style="height: 12px!important; width: 12px!important;" class="Parent"></canvas>
                                            </span>
                                            <span class="">Activity</span>
                                            <span>
                                                <canvas id="Canvas2" width="12" height="12" style="height: 12px!important; width: 12px!important;" class="Child"></canvas>
                                            </span>
    ...
    <script type="text/javascript" language="javascript">
        ...
        function projectActivityOperationDetails() {
            ...
            var ajaxData = {
                type: "POST",
                url: "frmDIProjectActivityOperationMatrix.aspx/getprojectActivityOperationDetails",
        ...
        function successProjectActivityOperationDetails(jsonData) {
            ...
            if (data.Table == "") {
                ...
                for (var v = 0, l = data.Table.length; v < l; v++) {
                    var inDataSet = [];
                    inDataSet.push(data.Table[v].vProjectActivityOperationId, data.Table[v].vUserTypeName, data.Table[v].vUserName, data.Table[v].vParentNodeDisplayName, data.Table[v].vNodeDisplayName,
                        data.Table[v].vMySubjectNo, data.Table[v].iModifyBy, data.Table[v].dModifyOn, data.Table[v].iParentNodeId, '');
                ...
                otable = $('#tblProjectActivityOperationMatrix').dataTable({
                    ...
                    "fnCreatedRow": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                        $('td:eq(6)', nRow).append('<a title="Delete" attrid="' + aData[0] + '" " Onclick=DeleteProjectActivityOperationMatrix(this) id="' + aData[0] + '" style="cursor:pointer;"><img src="Images/i_delete.gif" alt="Delete"></a>');
                        if (aData[8] == 1) {
        ...
        function projectActivityOperationSaveDetails() {
            var vWorkSpaceId = $("#ctl00_CPHLAMBDA_HProjectId").val()
            var iPeriod
            if ($("#ctl00_CPHLAMBDA_ddlPeriods").val() == "ALL") {
                iPeriod = 1;
            ...
            var ajaxData = {
                ...
                datatype: JSON,
                async: false,
                success: successProjectActivityOperationSaveDetails,
                error: errorProjectActivityOperationSaveDetails
            }
            getProjectActivityOperationDetails(ajaxData.type, ajaxData.url, ajaxData.data, ajaxData.contentType, ajaxData.datatype, ajaxData.async, ajaxData.success, ajaxData.error)
        }

        function successProjectActivityOperationSaveDetails(jsonData) {
            var data = JSON.parse(jsonData.d);
        }

        function errorProjectActivityOperationSaveDetails(e) {
            msgalert("ERROR : " + e)
        }
        ...
        $("[id*=tvUserType] input[type=checkbox]").live("click", function () {
            ...
            if (table.next().length > 0 && table.next()[0].tagName == "DIV") {
                ...
                if (chkNot != 0) {
                    $("#ctl00_CPHLAMBDA_tvUserType").find("table [type=checkbox]").first().removeAttr("checked");
                }
                ...
                var parentDIV = $(this).closest("DIV");
                if ($(this).closest("DIV").find("table [type=checkbox]").length == chkAll) {
                ...
                $("#ctl00_CPHLAMBDA_tvUserType").find("table [type=checkbox]").each(function () {
                    if ($(this).find("table [type=checkbox]").attr("checked") == true) {
                        IndexAll = IndexAll + 1;
                    }
                });
                if ($("#ctl00_CPHLAMBDA_tvUserType").find("table [type=checkbox]").length == IndexAll) {
                    $("#ctl00_CPHLAMBDA_tvUserType").find("table [type=checkbox]").first().attr("checked", "checked");
                } else {
                    $("#ctl00_CPHLAMBDA_tvUserType").find("table [type=checkbox]").first().removeAttr("checked")
                }
            }
        ...
        function ValidationForSave() {
            ...
            //    return false;
            //}
            projectActivityOperationSaveDetails();
            return true;
        }
        ...
        function ValidationForRemark(e) {
            ...
            msgConfirmDeleteAlert(null, "Do You Really Want to Delete This Record ?", function (isConfirmed) {
                if (isConfirmed) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "frmDIProjectActivityOperationMatrix.aspx/manageProjectActivityOperationDetails",
        ...
        function AuditTrail() {
            ...
            var ajaxData = {
                type: "POST",
                url: "frmDIProjectActivityOperationMatrix.aspx/AuditTrail",
                data: '{"vWorkSpaceId":"' + vWorkSpaceId + '","iPeriod":"' + iPeriod + '"}',
                contentType: "application/json; charset=utf-8",
                datatype: JSON,
                async: false,
        ...
        function exportPDF() {
            ...
            $.ajax({
                type: "POST",
                url: 'frmDIProjectActivityOperationMatrix.aspx/exportToPDF',
                data: '{"vWorkSpaceId":"' + vWorkSpaceId + '","iPeriod":"' + iPeriod + '","ProjectNo":"' + txtProject.value + '"}',
                contentType: "application/json; charset=utf-8",
                datatype: JSON,
                success: function (result) {
                    //var fileName = document.getElementById('tblProjectActivityOperationMatrix').rows[parseInt(0) + 2].cells[1].innerText + ".pdf"
                    //Convert Base64 string to Byte Array.
                    var fileName = "Project Activity Operation rights Audit Trail.pdf"
        ...
        function successProjectActivityOperationMatrixAuditTrailDetails(jsonAuditTrailData) {
            ...
            if (data.Table == "") {
                ...
                for (var v = 0, l = data.Table.length; v < l; v++) {
                    var inDataSet = [];
                    inDataSet.push(data.Table[v].vProjectActivityOperationId, data.Table[v].vUserTypeName, data.Table[v].vUserName, data.Table[v].vParentNodeDisplayName, data.Table[v].vNodeDisplayName,
                        data.Table[v].vMySubjectNo, data.Table[v].iModifyBy, data.Table[v].dModifyOn, data.Table[v].vRemark);
                    activityDataSet.push(inDataSet);
                }
