...
class App extends React.Component {
    ...
    {
        debugger;
        document.getElementById('lblImageTransmittal').innerHTML = "Test Upload";
        document.getElementById('lblImageTransmittalDetails').innerHTML = "Test Upload Details";
        this.filesize=0;
        this.rowCount=0;
        ...
        var minyear = dtToday.getFullYear() - 3;
        if(maxMonth < 10)
            maxMonth = '0' + maxMonth.toString();
        if(minMonth == '0')
        {
            minyear = dtToday.getFullYear() - 3;
            minMonth ='07';
        }
        if(minMonth < 10)
...
onKeyDownHandler = e => 
{
    //if(e.target.value.length >= 3)
    {
        ...
        axios({
            method: 'Post',
            url: 'frmUploadCertificateWeb.aspx/GetMyChildProjectCompletionList',
            data: data
        }).then(
            function (response) {
            // handle success
...
setProject = (HProjectId) =>
{
    ...
    axios({
        method: 'Post',
        url: 'frmUploadCertificateWeb.aspx/btnSetProject',
        data: data
    }).
    then(
...
bindSubject =(HProjectId) =>
{
    ...
    axios({
        method: 'Post',
        url: 'frmUploadCertificateWeb.aspx/FillSubjectDropDown',
...
bindModality = () =>
{
    ...
    axios({
        method: 'Post',
        url: 'frmUploadCertificateWeb.aspx/FillModalityDropDown',
        data: {}
    }).
    then(
    function (response) {// handle success   
...
bindAnatomy = () =>
{
    ...
    axios({
        method: 'Post',
        url: 'frmUploadCertificateWeb.aspx/FillAnatomyDropDown',
        data: {}
    }).
    then(
    function (response) {// handle success  
        ...
        for(var i=0 ; i < eval(response.data.d).length ; i++)
        {
            if(eval(response.data.d)[i].vAnatomyDesc == "Other"){
        ...
        for (var iindexx = 0; iindexx < fileData.files.length; iindexx++) {
            ...
            if(SingleFile.name != 'Thumbs.db')
            {
                ...
                this.FileListSaveData.push({
                    vFileName: FilesName,
                    vServerPath: "/" + HProjectId.value + "/" + this.FolderNameModalityWise + "/" + this.MaxImageiTranNo + "/Uploaded/" + FilesName,
                    vFileType:ext,
                ...
                this.FromPath = "\\" + HProjectId.value + "\\" + this.FolderNameModalityWise + "\\" + this.MaxImageiTranNo + "\\Uploaded\\";
            ...
            if(fileData.files.length >= 1){
                ...
                for (var iindex = 0; iindex < fileData.files.length; iindex++) {
                    ...
                    if(SingleFile.name != 'Thumbs.db')
                    {
                        ...
                        this.FileListSaveData.push({
                            vFileName: FilesName,
                            vServerPath: "/" + HProjectId.value + "/" + ddlSubject.value + "/" + this.FolderNameModalityWise + "/" + this.MaxImageiTranNo + "/Uploaded/" + FilesName,
                            vFileType:ext,
                            vSize: this.bytesToSize(file.size),
                            dScheduledDate:new Date().format('MM-dd-yyyy HH:mm'),
                        ...
                        this.FromPath = "\\" + HProjectId.value + "\\" + ddlSubject.value + "\\" + this.FolderNameModalityWise + "\\" + this.MaxImageiTranNo + "\\Uploaded\\";
                        this.fileUpload.files.push(SingleFile);
                    }
                }
                this.AllFileUpload.push.apply(this.AllFileUpload, this.fileUpload.files);
...
processFileData = (loopCount,flagCount_1) => {
    //for(var j=0 ; j<this.AllFileUpload.Allfiles.length ; j++){
        for (var i = loopCount; i < flagCount_1; i++) 
        {
            ...
            var LocalFilePath = "/" + HProjectId.value + "/" + this.FolderNameModalityWise + "/" + this.MaxImageiTranNo + "/Uploaded/";
    ...
    var readEventHandler = function (evt) {
        ...
        if (offset >= fileSize) {
            ...
            //if(DicomTagChangeYes.checked == true){
                self.DicomTagChanges(fileName, self.FromPath);
            //}
...
DicomTagChanges = (FileName, FromPath) =>
{
    const data ={ fileName: FileName, fromPath: DicomFilePath + FromPath }; const me=this;
    //axios({
    //    method: "Post",
    ...
    $.ajax({
        type: "POST",
        url: DISoftAPIURL + "SetData/CertDicomTagChange",
        data: data,
        async: false,
        dataType: "json",
...
GetVisitIsReviewedGetMAXiImageTranNoAPI = () => {
    ...
    $.ajax({
        type: "POST",
        url: DISoftAPIURL + "CommonData/CertMAXiImageTranNo",
...
FileUploadDataAPI = (result, FileName, LocalFilePath, LineLoder,IsUploadComplete) => {
    ...
    $.ajax({
        type: "post",
        url: DISoftAPIURL + "SetData/CertificateFileUpload",
        data: File,
        async: false,
        dataType: "json",
...
ProgressBar = (fileName,LineLoder)  =>
{
    ...
    if(DivFileName == null || DivFileName.id != fileName)
    {
        ...
        //td.className = "trBorder";
        //tr.appendChild(td);
        document.getElementById('ProgressLine').append(tr);
    }
    else{
...
ValidationForAuthentication = () => 
{
    ...
    axios({
        method: 'Post',
        url: 'frmUploadCertificateWeb.aspx/ValidatePassword',
        data: data
    }).
    then(
        function (response) {// handle success  
...
SaveDataAPI = () => {
    ...
    if(document.getElementById("ExaminationDate").value != null || document.getElementById("ExaminationDate").value != "" || document.getElementById("ExaminationDate").value !=undefined){
        ...
        var saveData = {
            iCertificateMasterID: null,
            iCertificateMasterImgDtlId: null,
            vWorkspaceId: HProjectId.value,
            ...
            vServerPath: "/" + HProjectId.value + "/" + this.FolderNameModalityWise + "/" + this.MaxImageiTranNo + "/Uploaded/",
            iImageTranNo: this.MaxImageiTranNo
        }
        ...
        //}
        //else{
        saveData["_SaveCertificate"] = this.FileListSaveData;
        //}
        const me = this;
        $.ajax({
            type: "POST",
            url: DISoftAPIURL + "SetData/InsertUpdate_Certificate",
            data: saveData,
            async: false,
            success: function (data) {
                console.log("Success");
                window.onUpdated();
                if(data == '-1'){
                    alert("Please Complate previous");
                    location.reload();
                }else{
                document.getElementById('myModalSignAuth').style.display = 'none';
                //if(j==1){
...
{     
    debugger;
    const FillGridData ={ WorkspaceId:  HProjectId.value},me = this;
    axios({
        method: 'Post',
        url: 'frmUploadCertificateWeb.aspx/FillGrid',
        data: FillGridData
    }).
    then(
    function (response) {// handle success 
        ...
        var tempTable = "";
        tempTable = " <thead> <tr class='trBorder'><th class='trBorder'>Audit Trail</th><th class='trBorder'>Subject</th>"+
                    " <th class='trBorder'>Total Image</th><th class='trBorder'>Review status</th><th class='trBorder'>Change On</th></tr></thead><tbody>";
        if(response.data.d != null){
            for(var i=0;i < eval(response.data.d).length; i++)
            {
                tempTable +="<tr class='trBorder'><td><center><img alt='submit' style ='cursor: pointer' src='images/audit.png' onclick='HandleAuditTrail(" + eval(response.data.d)[i].iCertificateMasterID + ")'></img></center></td><td class='trBorder'>" 
                    + eval(response.data.d)[i].vSubjectId + "</td><td class='trBorder'>" 
                    + eval(response.data.d)[i].iNoImages + "</td><td class='trBorder'>" 
                    + eval(response.data.d)[i].cReviewStatus + "</td><td class='trBorder'>"
                    + eval(response.data.d)[i].ChangeOn + "</td>";
            }
        }
...
VisitValidation = () => {
    debugger;
    const VisitValidateData ={ iProjectId:  HProjectId.value}
    axios({
        method: 'Post',
        url: 'frmUploadCertificateWeb.aspx/VisitValidation',
    ...
    function (response){
        ...
        for (var i = 0; i < 1; i++) {
            ...
            if (ddlModality.options[ddlModality.selectedIndex].outerText == eval(response.data.d)[i].vModalityDesc && eval(response.data.d)[i].cReviewStatus == "Approved")
            {
                alert(" Modality '" + eval(response.data.d)[i].vModalityDesc + "' are under the review");
                document.getElementById('ddlModality').value = "0";
            }
        }
    }).catch(
...
handleVisitSet = (VisitName) =>
{
    ...
    if(VisitName != '')
    {
        const data = {HProjectId : HProjectId.value,SubjectId : HSubjectId.value},me=this;
        axios({
            method: 'Post',
            url: 'frmUploadCertificateWeb.aspx/CheckVisitStatus',
            data: data
        }).
        then(
            function (response) {// handle success  
                                    ...
                                    <table style={{width: '99%', margin: 'auto'}}>
                                        <tbody>
                                            <tr>
                                                ...
                                                <td style={{textAlign: 'left',width: '30%'}} colSpan="1">
                                            ...
                                            <tr style={{height: '2pc'}}>
                                                ...
                                                <td style={{textAlign: 'left'}}>
                                                    <label style={{color: 'green'}}><input type="radio" id="contrastYes" name="contrast" style={{margin:'revert'}} defaultChecked /> Yes </label>
                                                ...
                                                <td style={{textAlign: 'left'}}>
                                                    <label style={{color: 'green'}}><input type="radio" id="OralcontrastYes" name="Oralcontrast" style={{margin:'revert'}} defaultChecked /> Yes </label>
